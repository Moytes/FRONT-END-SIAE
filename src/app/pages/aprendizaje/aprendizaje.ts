...
import { AprendizajeService } from '../../core/services/aprendizaje.service';
...
export class Aprendizaje implements OnInit {
  private aprendizajeService = inject(AprendizajeService);
  ...
  primaryStudentsCount = computed(() => this.students().filter(student => (student.groupName || '').includes('°')).length);
  ...
  loadInitialData(): void {
    ...
    this.aprendizajeService.getSchools().subscribe({
      next: schools => {
  ...
  loadYears(): void {
    this.aprendizajeService.getSchoolYears().subscribe({
      next: years => {
  ...
  loadGroups(): void {
    this.aprendizajeService.getGroups(this.selectedSchoolId(), this.selectedYearId()).subscribe({
  ...
  loadStudents(): void {
    this.loading.set(true);
    this.aprendizajeService.getStudents(undefined, this.selectedSchoolId(), this.selectedGroupId()).subscribe({
      next: students => {
      ...
      error: err => {
        this.loading.set(false);
        this.handleError(err, 'No se cargaron los alumnos de aprendizaje');
      }
    });
  }
